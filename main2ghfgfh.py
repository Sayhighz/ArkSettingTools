...
def showMenu():
    ...
    mainMenu.add_command(label = "Restart", command=reload_game)
...
def restartGame():
    ...
    ark_exe = lablePath.get() + '/ShooterGame/Binaries/Win64/ShooterGame_BE.exe'
...
def reload_game_thread():
    ...
    for p in psutil.process_iter():
        if p.name() == "ShooterGame.exe":
            #status_label.config(text="Status: Killing ARK Game", fg="red")
        ...
        pass
        #status_label.config(text="Status: ARK Game not running", fg="red")
    
    # Reload the game
    #status_label.config(text="Status: Reloading ARK Game", fg="orange")
    ...
    while True:
        for p in psutil.process_iter():
            if p.name() == "ShooterGame.exe":
                #status_label.config(text="Status: ARK Game Reloaded", fg="green")
                return
        time.sleep(1)    
